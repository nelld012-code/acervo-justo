...
function AuthenticatedLayout() {
  return <SidebarProvider><div className="flex min-h-screen w-full overflow-x-hidden bg-background"><AppSidebar /><div className="flex min-h-screen min-w-0 flex-1 flex-col"><header className="flex min-h-14 items-center gap-2 border-b bg-card px-3 sm:px-4"><MobileMenuTrigger /><div className="min-w-0 flex flex-1 flex-col justify-center"><h1 className="truncate text-base font-semibold leading-tight text-foreground md:text-lg lg:text-xl md:whitespace-nowrap"><span className="md:hidden">Gestão de Documentos Judiciais</span><span className="hidden md:inline">J DIMAS GONÇALVES ESCRITORIO DE ADVOCACIA</span></h1><div className="hidden text-xs leading-tight text-muted-foreground md:block lg:text-sm">Sistema de Gestão de Documentos Judiciais</div></div><HeaderActions /></header><main className="min-w-0 flex-1 overflow-x-hidden p-4 sm:p-6"><Outlet /></main><footer className="border-t bg-card px-4 py-3 text-center text-xs text-muted-foreground sm:text-sm">© 2026 Desenvolvido por: Michel Antonio Alvarado</footer></div></div><PrazoReminders /><NewMessagePopup /></SidebarProvider>;
}
