...
import { Bell, CalendarPlus, ChevronLeft, ChevronRight, ExternalLink, FileSpreadsheet, Pencil, Printer, Trash2, X } from "lucide-react";
  ...
  return (
    <div className="min-w-0 space-y-4">
      <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
        ...
        <div className="flex flex-wrap gap-2"><Button variant="outline" onClick={() => fileRef.current?.click()}><FileSpreadsheet className="mr-2 h-4 w-4" />Importar Excel</Button><Button variant="outline" onClick={() => printLista(filtered)}><Printer className="mr-2 h-4 w-4" />Imprimir lista</Button><Button onClick={openNew}><CalendarPlus className="mr-2 h-4 w-4" />Nova audiência</Button><input ref={fileRef} type="file" accept=".xlsx,.xls" className="hidden" onChange={e => void importFile(e.target.files?.[0])} /></div>
      </div>
