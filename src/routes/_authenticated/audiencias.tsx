  ...
  return (
    <div className="min-w-0 space-y-4">
      <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
        ...
        <div className="flex flex-col gap-2 sm:flex-row"><Button onClick={() => printLista(filtered)} variant="outline" className="min-h-11 w-full sm:w-auto"><Printer className="mr-2 h-4 w-4" />Imprimir lista</Button><Button variant="outline" onClick={() => fileRef.current?.click()} className="min-h-11 w-full sm:w-auto"><FileSpreadsheet className="mr-2 h-4 w-4" />Importar Excel</Button><Button onClick={openNew} className="min-h-11 w-full sm:w-auto sm:ml-auto"><CalendarPlus className="mr-2 h-4 w-4" />Nova audiência</Button><input ref={fileRef} type="file" accept=".xlsx,.xls" className="hidden" onChange={e => void importFile(e.target.files?.[0])} /></div>
      </div>
