...
type Audiencia = {
  ...
  cidade_lugar: string | null; local_audiencia: string | null; link_virtual: string | null; observacao: string | null;
...
const EMPTY_FORM = {
  ...
  cidade_lugar: "", local_audiencia: "", link_virtual: "", observacao: "", status: "Agendada" as (typeof STATUS)[number],
...
function printAudiencia(a: Audiencia) {
  printReport({ title: "Registro de Audiência", subtitle: "J DIMAS GONÇALVES · ESCRITORIO DE ADVOCACIA", sections: [{ heading: "Dados da audiência", columns: ["Campo", "Informação"], rows: [
    ...
    ["Tipo", a.tipo_audiencia], ["Modalidade", a.modalidade], ["Lugar / Cidade", a.cidade_lugar || "—"], ["Local", a.local_audiencia || "—"], ["Link Virtual", a.link_virtual || "—"], ["Status", a.status], ["Observação", a.observacao || "—"],
  ] }] });
}
...
function AudienciasPage() {
  ...
  const filtered = useMemo(() => {
    ...
    return audiencias.filter(a => {
      if (q && ![a.nome, a.numero_processo ?? "", a.parte ?? "", a.advogado ?? "", a.orgao_julgador ?? "", a.vara ?? "", a.cidade_lugar ?? ""].some(v => v.toLowerCase().includes(q))) return false;
  ...
  const save = useMutation({
    mutationFn: async () => {
      ...
      const payload = {
        ...
        vara: form.vara.trim() || null, tipo_audiencia: form.tipo_audiencia, modalidade: form.modalidade,
        cidade_lugar: form.cidade_lugar.trim() || null, local_audiencia: form.local_audiencia.trim() || null,
  ...
  function openEdit(a: Audiencia) {
    ...
    setForm({
      ...
      tipo_audiencia: a.tipo_audiencia, modalidade: a.modalidade, cidade_lugar: a.cidade_lugar ?? "", local_audiencia: a.local_audiencia ?? "", link_virtual: a.link_virtual ?? "",
